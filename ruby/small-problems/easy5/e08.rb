# Alphabetical Numbers

INTEGER_WORDS = %w(zero one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen)


def alphabetic_number_sort(integer_array)
  integers_as_words = integer_array.map { |int| INTEGER_WORDS[int] }
  integers_as_words.sort.map { |word| INTEGER_WORDS.index(word) }
end

def given_solution(numbers)
  numbers.sort_by { |number| INTEGER_WORDS[number] }
end

puts alphabetic_number_sort((0..19).to_a) == [
  8, 18, 11, 15, 5, 4, 14, 9, 19, 1, 7, 17,
  6, 16, 10, 13, 3, 12, 2, 0
]
