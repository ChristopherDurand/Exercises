...
function dateFromTextData(dateString) {
  let [month, day, year] = dateString.split('-').map(Number);
  return new Date(year, month - 1, day);
}

function parseEmailObject(emailString) {
  let fields = emailString.split('#/#\n')
  return {
    from: fields[0].slice(6),
    subject: fields[1].slice(9),
    date: dateFromTextData(fields[2].slice(6)),
    to: fields[3].slice(3),
    body: fields[4],
  };
}
...
function parseDateRange(emailData) {
  let dates = emailData.map(({date}) => date);
  let earliest = new Date(Math.min(...dates));
  let latest = new Date(Math.max(...dates));
  return `${prettyDateString(earliest)} - ${prettyDateString(latest)}`
}

function mailCount(emailString) {
  let emails = emailString.split('##||##\n\n').map(parseEmailObject);
  console.log(`Count of Email: ${emails.length}`);
  console.log(`Date Range: ${parseDateRange(emails)}`);
}
